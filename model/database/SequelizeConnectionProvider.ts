...
export class SequelizeConnection implements DatabaseProvider {
  ...
  constructor(public params: ISystemParameters) {
    ...
    try {
      this.bdConnection = new Sequelize(
        this.params.bd_name,
        this.params.bd_user,
        this.params.bd_pass,
        {
          port: Number(this.params.bd_port),
  ...
  async insert(preparedInsert: PreparedInsert): Promise<[number, string]> {
    try {
      const [a] = await this.bdConnection.query(
        `INSERT INTO ${preparedInsert.table} 
            ( ${Object.keys(preparedInsert.params)
              .map((el) => el)
              .join(",")} ) 
            VALUES (  ${Object.keys(preparedInsert.params)
              .map((el) => `:${el}`)
              .join(",")} ) `,
        {
  ...
  async update(preparedUpdate: PreparedUpdate): Promise<[number, string]> {
    try {
      const query = `UPDATE ${preparedUpdate.table} 
            SET ${Object.keys(preparedUpdate.params)
              .map((el) => `${el} = :${el}`)
              .join(",")} 
